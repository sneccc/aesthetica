...
def autoclip_aggregation(cosine_similarities, beta=0.85):
    instance_multiple, n_classes, n_descriptors = cosine_similarities.shape[:3]
    
    # Initialize weights uniformly (weights used for weighted class descriptor averaging)
    weights_logits = torch.nn.parameter.Parameter(
        torch.zeros((instance_multiple, n_classes, n_descriptors), device=cosine_similarities.device)
    )
    weights_logits.requires_grad = True
    weights = weights_logits.softmax(dim=2)  # (icd)

    # Compute log-sum-exp of class logits and its gradient wrt. the weight's logits
    logits = torch.sum(weights[:, :, None] * cosine_similarities, -1)  # (icg)
    lse = torch.logsumexp(logits, axis=2).sum()
    grad_weights_logits = torch.autograd.grad(lse, weights_logits)[0]

    # Determine target entropy (compare Section 3.4 of the paper)
    base_entropy = -(weights * torch.log2(weights + 1e-10)).sum(-1)  # (ic)
    target_entropy_step = base_entropy * beta

    # Select step sizes based on line search to match target_entropy
    step_sizes = torch.ones((instance_multiple, n_classes), device=cosine_similarities.device)
    for i in range(instance_multiple):
        for j in range(n_classes):
            def f(step_size):
                weights_logits_ = weights_logits[i, j] + step_size * grad_weights_logits[i, j]
                weights_ = torch.softmax(weights_logits_, dim=-1)
                entropy = -(weights_ * torch.log2(weights_ + 1e-10)).sum(-1)
                return entropy.item() - target_entropy_step[i, j].item()

            step_sizes[i, j] = torch.tensor(bisect(
                f=f,
                a=0.0,
                b=1e7,
                maxiter=100,
